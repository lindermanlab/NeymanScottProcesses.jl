...
function recompute!(
    ...
    for (s, k) in zip(datapoints, assignments)
        ...
        while k > length(ev.events)
            push!(
                ev.events,
                ev.constructor(constructor_args(ev)...)
            )
        end
