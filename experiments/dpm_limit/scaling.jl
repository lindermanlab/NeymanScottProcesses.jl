...
theme(:dao; label=nothing, titlefontsize=8, guidefontsize=8, labelfontsize=8)
...
function fit_data(config)
	...
	save_keys = (:log_p, :log_joint, :time, :assignments)
...
iwmean(0.0005 * 100.0, 100.0)  # Interpolate?
...
# ╔═╡ 4d27b80e-8591-42af-82fc-16e5254c75c4
n
...
# ╔═╡ 8db45758-e677-496c-8297-7255f078a68e
...
# ╔═╡ a458f37f-6d38-464f-9848-400f90a67e17
figheight = 200

# ╔═╡ 6f61ef8a-b65f-4df1-94f8-e3f184762ea9
pagewidth = 650

# ╔═╡ efe74528-c4f6-47ac-a597-f416727c7dce
plt_data = plot(
	plot_data(cov_scale=0.001, df=5.0, title="Main Synthetic Experiment"),
	plot_data(cov_scale=0.05^2 * 100, df=100.0, title="Dimension Scaling Experiment"),
	layout=(1, 3),
	size=(pagewidth/2, 200),
)
...
# ╔═╡ 7940a2f3-871c-4c8a-8b45-2998c825bd73
200/650

# ╔═╡ 2aa6c1c6-8dfa-48e1-8ff3-af3933c1e1a3
final_plot = let
	
	
	plt_data_1 = plot_data(cov_scale=0.001, df=5.0, title="Main Experiment")
	plt_data_2 = plot_data(cov_scale=0.05^2 * 100, df=100.0, title="Dimension Experiment")
	...
	plt = plot(
		...
		plt_dim,
		layout=lo, #(1, 3),
		size=(pagewidth, figheight)
	)

	δx = -0.02
	δy = 0.075

	annotate!(plt[1], δx, 1 + δy, text("A", "Times Bold", 12))
	annotate!(plt[2], δx, 1 + δy, text("B", "Times Bold", 12))
	annotate!(plt[3], 20*(3/4)*δx, 1550 + 1600*δy, text("C", "Times Bold", 12))

	plt
end

# ╔═╡ 563ba28b-fc66-4fa8-aa7f-5d72163a5ed8
savefig(final_plot, "../../figures/dimension_experiment.pdf")
